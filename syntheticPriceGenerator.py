...
#     # Precompute the total required log return
#     total_log_return = np.log(close_price / open_price)
    
#     # Generate random returns and adjust them to match the total log return
#     returns = np.random.normal(0, sigma / np.sqrt(n_steps), n_steps - 1)
#     adjustment = (total_log_return - np.sum(returns)) / (n_steps - 1)
#     adjusted_returns = returns + adjustment
    
#     # Compute cumulative log returns and convert to prices
#     cum_log_returns = np.insert(adjusted_returns.cumsum(), 0, 0)  # Start at 0
#     prices = open_price * np.exp(cum_log_returns)
